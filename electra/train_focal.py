...
def save_checkpoint(state, is_best, save_path, model_name='checkpoint'):
    """
    Save model checkpoint to file.
    
    Args:
        state (dict): State dictionary containing model weights and other info
    ...
    latest_path = os.path.join(save_path, f'{model_name}_latest.pth')
...
def lcl_train(log):
    ...
    losses = {"contrastive":loss.LCL(temperature=log.param.temperature),"emotion":FocalLoss(),"lambda_loss":log.param.lambda_loss}
    ...
    for epoch in range(1, log.param.nepoch + 1):
        ...
        checkpoint_helper = {
            'epoch': epoch,
            'state_dict': model_helper.state_dict(),
            'optimizer': optimizer.state_dict(),
            'best_criterion': best_criterion,
            'valid_acc': val_acc_2,
            'test_acc': test_acc_2
        ...
        # Save both models
        save_checkpoint(checkpoint_main, is_best, save_home, model_name='model_main')
        save_checkpoint(checkpoint_helper, is_best, save_home, model_name='model_helper')
